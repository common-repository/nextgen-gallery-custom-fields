...
Plugin URI: http://shauno.co.za/wordpress/nextgen-gallery-custom-fields/
...
//{
	...
	//install funcs{
		...
		function nggcf_install($upgrade=false) {
			...
			if($upgrade) {
				...
				if (method_exists(\Imagely\NGG\DataMappers\Gallery::class, 'get_instance')) {
					$gallery_mapper = \Imagely\NGG\DataMappers\Gallery::get_instance();
					$nggGalleries = $gallery_mapper->find_all();
				} else {
					global $nggdb;
	...
	//admin stuff{
		...
		//NGG >= v2.0.57
		add_filter('ngg_manage_images_number_of_columns', 'nggcf_add_image_cols');
		...
		function nggcf_image_options($type) {
			...
			if(is_numeric(isset($_GET["delete"]) && $_GET["delete"])) {
				...
				<table cellspacing="0" class="widefat fixed" id="table-existing">
					...
					foreach ((array)$fields as $key=>$val) {
						...
						<tr>
							<td>
								<?php
								if(isset($_GET["editname"]) && $_GET["editname"] == $val->id) { 
									$fstyle = "block";
									$nstyle = "none";
								}else{
									$fstyle = "none";
		...
		function nggcf_add_image_cols($numCols) {
			...
			foreach ((array)$fields as $key=>$val) {
				...
				add_filter('ngg_manage_images_column_'.$numCols.'_header', 'nggcf_image_col_header', $numCols);
				add_filter('ngg_manage_images_column_'.$numCols.'_content', 'nggcf_image_col_field', 10, 2);
			}

			return $numCols;
		}
		
		function nggcf_image_col_header($name) {
			global $nggcf_image_cols;
			static $colsList;
			$colsList = $nggcf_image_cols;
			$col = array_shift($nggcf_image_cols);
			
			return htmlspecialchars($col->field_name);
		}
		...
		function nggcf_image_col_field($output='', $picture=array()) {
			...
			ob_start();
			nggcf_admin_col($col->field_name, $picture->pid);
		...
		add_filter('ngg_manage_gallery_fields', 'nggcf_add_gallery_col', 11, 2);
		function nggcf_add_gallery_col($fields=array(), $gallery=NULL) {
			$fields['left']['ngg_cf_options'] = array(
				'callback'	=> 'nggcf_admin_gallery_col',
		...
		function nggcf_admin_gallery_col($gallery=array()) {
			global $wpdb;
			$fields = nggcf_get_field_list(NGGCF_GALLERY, $gallery->gid);
			
			echo '<strong>Custom Columns</strong><hr /><input type="hidden" name="nggcf_gallery[ngg_gallery_id]" value="'.$gallery->gid.'" />';
			
			foreach ((array)$fields as $key=>$val) {
				
				echo htmlspecialchars($val->field_name).'<br />';
				
				
				$value = $wpdb->get_row("SELECT * FROM ".$wpdb->prefix."nggcf_field_values WHERE pid = '".$wpdb->escape($gallery->gid)."' AND fid = '".$val->id."' AND ngg_type = '".NGGCF_GALLERY."'");
				switch($val->field_type) {
				case NGGCF_FIELD_TYPE_INPUT :
